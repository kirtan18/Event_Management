...
const { execute, subscribe } = require('graphql');
const { SubscriptionServer } = require('subscriptions-transport-ws');
const { schema, pubsub } = require('./src/component/schema');
const responseGenerator = require('./src/helper/responseGenerator');
...
app.use(
  ...
  graphiqlExpress({
    endpointURL: '/graphql',
    subscriptionsEndpoint: `ws://localhost:${serverPort}/graphql`,
    graphiql: true,
  })
);
...
server.listen(serverPort, () => {
  ...
  console.info(`GraphQL server running on http://localhost:${serverPort}`);
});
